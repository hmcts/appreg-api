...
Never commit secrets or tokens (for example: `AZURE_TENANT_ID`, `AZURE_AUDIENCE_ID`, `AZURE_ISSUER`, DB passwords, `SONAR_TOKEN`, `AZURE_DEVOPS_ARTIFACT_TOKEN`). Do not include secrets, tokens, credentials, PII, runner file contents, environment variables, or auth material in patches, PR bodies, comments, logs, artifacts, fixtures, or tests. Prefer environment variables or secret mounts (`/mnt/secrets/appreg/`). For local DB parity, use docker compose profiles (`testing` or `functionaltesting`) and keep credentials in local-only env files. Run dependency scanning (`./gradlew dependencyCheck`) before release-sensitive changes.
