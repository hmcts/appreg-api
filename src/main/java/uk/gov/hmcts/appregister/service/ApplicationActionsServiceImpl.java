...
public class ApplicationActionsServiceImpl implements ApplicationActionsService {
    ...
    public void moveApplications(List<Long> applicationIds, Long targetListId, String userId) {
        List<Application> applications = applicationRepository.findByIdInAndApplicationListUserId(applicationIds, userId);

        if (applications.size() != applicationIds.size()) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "One or more applications are not accessible by this user");
        }

        // TODO: Should we all user to create a new list if the target list doesn't exist?
        ApplicationList targetList = applicationListRepository.findByIdAndUserId(targetListId, userId)
            .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Target application list not found"));
