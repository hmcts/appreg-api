...
public class ApplicationEntryResultCreationValidator
    ...
    PayloadForCreateEntryResult<ResultCreateDto>, ListEntryResultCreateValidationSuccess> {
    ...
    public <R> R validate(
        ...
        BiFunction<PayloadForCreateEntryResult<ResultCreateDto>, ListEntryResultCreateValidationSuccess, R>
            validateSuccess) {
        ...
        ResolutionCode resolutionCode = validateResolutionCode(validatable.getData().getResultCode());

        // 4) Parse wording template (throws if malformed)
        WordingTemplateSentence wordingTemplate = WordingTemplateSentence.with(resolutionCode.getWording());
    ...
    private ApplicationListEntry validateEntry(UUID entryId, ApplicationList list) {
        Optional<ApplicationListEntry> entry =
            applicationListEntryRepository.findActiveByUuidAndApplicationListUuid(entryId, list.getUuid());
    ...
    private ResolutionCode validateResolutionCode(String resolutionCode) {
        Optional<ResolutionCode> code =
            resolutionCodeRepository.findByResultCodeIgnoreCase(resolutionCode);
