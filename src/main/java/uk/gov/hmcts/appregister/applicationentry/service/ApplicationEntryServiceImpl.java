...
public class ApplicationEntryServiceImpl implements ApplicationEntryService {
    ...
    @Override
    @Transactional
    public MatchResponse<EntryGetDetailDto> createEntry(PayloadForCreate<EntryCreateDto> entryCreateDto) {
        log.debug(
            "Started: Create Application Entry: {}",
            entryCreateDto);

        // creates the entity and return the etag for matching
        MatchResponse<EntryGetDetailDto> getDetailDto = createApplicationEntryValidator.validate(entryCreateDto,
            (dto, success) -> {
                return auditService.processAudit(
                    ...
                    req -> {
                        ...
                        if (entryCreateDto.getData().getApplicant() != null) {
                            applicantToSave = applicationListEntryEntityMapper
                                .mapApplicant(entryCreateDto.getData().getApplicant());
                            nameAddressRepository.save(applicantToSave);
                            log.debug("Created applicant with id: {}", applicantToSave.getId());
                        }

                        // save the respondent
                        NameAddress respondentToSave = null;
                        if (entryCreateDto.getData().getRespondent() != null) {
                            respondentToSave = nameAddressRepository.save(
                                applicationListEntryEntityMapper.mapRespondent(
                                    entryCreateDto.getData().getRespondent()));
                            log.debug("Created respondent with id: {}", respondentToSave.getId());
                        }

                        // save the list
                        ApplicationListEntry listEntryEntity
                            = applicationListEntryEntityMapper.toApplicationListEntry(entryCreateDto.getData(),
                        ...
                        listEntryEntity = applicationListEntryRepository.save(listEntryEntity);
                        ...
                        for (FeeStatus feeStatus : entryCreateDto.getData().getFeeStatuses()) {
                            AppListEntryFeeStatus createdAppListStatus = appListEntryFeeStatusRepository
                                .save(applicationListEntryEntityMapper.toFeeStatus(feeStatus, listEntryEntity));
                            statusList.add(createdAppListStatus);
                            log.debug("Fee status created and mapped to application entry with id: {}", createdAppListStatus.getId());
                        }
                        ...
                        for (Official official : entryCreateDto.getData().getOfficials()) {
                            AppListEntryOfficial createdOriginal = appListEntryOfficialRepository.save(applicationListEntryEntityMapper
                                                                    .toOfficial(official, listEntryEntity));
                            officialList.add(createdOriginal);
                            log.debug("Original created and mapped to application entry with id: {}", createdOriginal.getId());
                        }

                        EntryGetDetailDto entryGetDetailDto = applicationListEntryMapStructMapper
                            .toEntryGetDetailDto(listEntryEntity,
                                                 statusList,
                                                 success.getFee(),
                                                 officialList);
                        ...
                        appListEntryFeeId = appListEntryFeeRepository.save(appListEntryFeeId);
                        log.debug("Created Fee: {} to Entry: {} mapping: {}", appListEntryFeeId.getFeeId(),
                                  appListEntryFeeId.getAppListEntryId().getEntryFeeIds());
        ...
        log.debug(
            "Finish: Create Application Entry: {}",
            entryCreateDto);
