...
public interface StandardApplicantRepository extends JpaRepository<StandardApplicant, Long> {
     ...
     *
     * Name can represent the name title or the forename_1 or the surname. If name is not null we use name as
     * this is an organisation. The expectations is that the forename and surname will be null in this case.
     *
     * If the name is not null then we search matching results on the name, forename_1 and surname fields.
     *
    ...
    @Query(
        ...
        WHERE (:code IS NULL OR c.applicantCode ILIKE CONCAT('%', CAST(:code AS string), '%'))
          ...
          AND ((:name IS NULL AND :surname IS NULL) OR (((:name IS NULL OR c.name IS NOT NULL AND :name IS NOT NULL AND c.name 
                  ...
                  OR (:name IS NULL OR c.applicantForename1 IS NOT NULL AND :name IS NOT NULL AND c.applicantForename1
                           ILIKE CONCAT('%', CAST(:name AS string), '%')))
                  AND (:surname IS NULL OR c.applicantSurname IS NOT NULL AND :surname IS NOT NULL AND c.applicantSurname
                           ILIKE CONCAT('%', CAST(:surname AS string), '%'))))
        """)
