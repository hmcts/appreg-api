 ...
 * <ul>
 *   <li>Reads an {@code X-Correlation-Id} header from the request, if present.</li>
 *   <li>If missing, generates a new UUID to act as the correlation ID.</li>
 *   <li>Stores the correlation ID on the request attributes (under {@code "correlationId"})
 *       so downstream components can access it.</li>
 *   <li>Puts the correlation ID into SLF4J's {@link MDC} so it appears in log statements
 *       for the current thread during request processing.</li>
 *   <li>Removes the MDC entry in a {@code finally} block to avoid leaking values across threads.</li>
 * </ul>
...
public class CorrelationFilter implements Filter {
    ...
    public void doFilter(ServletRequest request,
        ...
        // (e.g., controllers or other filters can read it with request.getAttribute("correlationId")).
