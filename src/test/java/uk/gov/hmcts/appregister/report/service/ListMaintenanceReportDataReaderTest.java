...
class ListMaintenanceReportDataReaderTest {
    @Test
    void givenReportRowsExist_whenReadData_thenReadsPagesWithExpectedParameters() throws Exception {
        ...
        List<List<ListMaintenanceReportRow>> pages = new ArrayList<>();
        ...
        when(jdbcTemplate.query(
                        ...
                        ArgumentMatchers.<RowMapper<ListMaintenanceReportRow>>any()))
                .thenAnswer(
                        invocation -> {
                            queries.add(invocation.getArgument(0));
                            parameterSources.add(invocation.getArgument(1));
                            RowMapper<ListMaintenanceReportRow> rowMapper =
        ...
        Assertions.assertEquals("OPEN", row.getListStatus());
        Assertions.assertEquals(69L, row.getApplicationEntryCount());
        ...
        assertLegacyListMaintenanceQueryShape(queries.getFirst());
    }
    ...
    void givenNoLocation_whenReadData_thenLocationParametersAreNull() throws IOException {
        ...
        Assertions.assertNull(parameters.getValue("listDescription"));
    ...
    private ListMaintenanceFilterDto filter() {
    ...
    private ResultSet resultSet() throws Exception {
        ...
        when(resultSet.getString("application_list_status")).thenReturn("OPEN");
        when(resultSet.getLong("application_entry_count")).thenReturn(69L);
        return resultSet;
    }
}
