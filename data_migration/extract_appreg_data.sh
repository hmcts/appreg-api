...
# 15.0		21/09/2026	Matthew Harman	Changes for the DATA_AUDIT
#						table as per ARCPOC-1826
#
...
#
# The next options refer to the DATA_AUDIT table
# DATA_AUDIT_INCLUDE		true|false	- is the DATA_AUDIT table to
#				be included in the extraction
DATA_AUDIT_INCLUDE=true;	
#
# DA_RETENTION			USE_SYSTEM|NONE|CUSTOM
#				What retention should be applied to the 
#				DATA_AUDIT table, takes 1 of three values:
#				USE_SYSTEM: 	Use the retention configured in
#						RETENTION_POLICY
#				NONE:		Don't apply retention to the
#						DATA_AUDIT table
#				CUSTOM:		Use a separate retention policy
#						to the DATA_AUDIT table
DA_RETENTION='CUSTOM';
#
# DA_CUSTOM_RETENTION		Retention period, used if DA_RETENTION=CUSTOM
DA_CUSTOM_RETENTION='TRUNC(SYSDATE-100)';
...
DATA_AUDIT_FIELDS="DATA_ID:NUMBER:NUMERIC:N:N:NONE:N;SCHEMA_NAME:VARCHAR:VARCHAR(32):N:N:NONE:N;TABLE_NAME:VARCHAR:VARCHAR(32):N:N:NONE:N;COLUMN_NAME:VARCHAR:VARCHAR(32):N:N:NONE:N;OLD_VALUE:VARCHAR:VARCHAR(4000):Y:N:DBMS_RANDOM.STRING('',NVL(LENGTH(OLD_VALUE),0)):N;NEW_VALUE:VARCHAR:VARCHAR(4000):Y:N:DBMS_RANDOM.STRING('',NVL(LENGTH(NEW_VALUE),0)):N;USER_ID:VARCHAR:VARCHAR(73):Y:N:NONE:N;LINK:VARCHAR:VARCHAR(100):Y:N:NONE:N;CREATED_DATE:TIMESTAMP:TIMESTAMP:N:N:NONE:N;OLD_CLOB_VALUE:CLOB:TEXT:Y:N:DBMS_RANDOM.STRING('',500):N;NEW_CLOB_VALUE:CLOB:TEXT:Y:N:DBMS_RANDOM.STRING('',500):N;RELATED_KEY:NUMBER:NUMERIC:Y:N:NONE:N;UPDATE_TYPE:VARCHAR:VARCHAR(1):N:N:NONE:N;DATA_TYPE:VARCHAR:VARCHAR(1000):Y:N:NONE:N;CASE_ID:NUMBER:NUMERIC:Y:N:NONE:N;RELATED_ITEMS_IDENTIFIER:VARCHAR:VARCHAR(30):Y:N:NONE:N;RELATED_ITEMS_IDENTIFIER_INDEX:VARCHAR:VARCHAR(30):Y:N:NONE:N;EVENT_NAME:VARCHAR:VARCHAR(100):Y:N:NONE:N;USER_NAME:VARCHAR:VARCHAR(250):Y:N:NONE:N";
...
do
	if [ ${tables_to_extract} == "APPREGISTER.DATA_AUDIT" ] && [ ${DATA_AUDIT_INCLUDE} = false ]; then
		# we don't do DATA_AUDIT as it is set to not include
		echo "DATA AUDIT not included";
	else
		sql_head="";
		sql_spool="";
		sql_script="";
...
echo "running case: $tables_to_extract $lower_table_name";
		case $tables_to_extract in
			...
			APPREGISTER.DATA_AUDIT)
				...
				shard_field="DATA_ID";
				changed_by='USER_ID';
				if [ ${DA_RETENTION} == "USE_SYSTEM" ]; then
					# Use the overriding retention
					retention_clause="trunc(created_date) > ${retention_policy}";
				elif [ ${DA_RETENTION} == "CUSTOM" ]; then
					# use specific DATA_AUDIT retention
					retention_clause="trunc(created_date) > ${DA_CUSTOM_RETENTION}";
				else
					# NONE, no retention
					retention_clause="";
				fi
...
middle_name TEXT,
last_name TEXT,";
				additional_oracle_select="REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(TO_CLOB(NVL(forename_1,'')),CHR(92),CHR(92)||CHR(92)),'|',CHR(92)||'|'),CHR(13),CHR(92)||'r'),CHR(10),CHR(92)||'n'),CHR(9),CHR(92)||'t')||CHR(31)||";
				additional_oracle_select="${additional_oracle_select}REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(TO_CLOB(TRIM(REGEXP_REPLACE(NVL(forename_2, '')|| ' ' || NVL(forename_3, ''), ' +', ' '))),CHR(92),CHR(92)||CHR(92)),'|',CHR(92)||'|'),CHR(13),CHR(92)||'r'),CHR(10),CHR(92)||'n'),CHR(9),CHR(92)||'t')||CHR(31)||";
				additional_oracle_select="${additional_oracle_select}REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(TO_CLOB(NVL(surname,'')),CHR(92),CHR(92)||CHR(92)),'|',CHR(92)||'|'),CHR(13),CHR(92)||'r'),CHR(10),CHR(92)||'n'),CHR(9),CHR(92)||'t')||CHR(31)||'#'";
	
				additional_oracle_select_masked="REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(TO_CLOB(DBMS_RANDOM.STRING('l',NVL(LENGTH(forename_1),0))),CHR(92),CHR(92)||CHR(92)),'|',CHR(92)||'|'),CHR(13),CHR(92)||'r'),CHR(10),CHR(92)||'n'),CHR(9),CHR(92)||'t')||CHR(31)||";
				additional_oracle_select_masked="${additional_oracle_select_masked}REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(TO_CLOB(DBMS_RANDOM.STRING('l',NVL(LENGTH(forename_2),0))||' '||DBMS_RANDOM.STRING('l',NVL(LENGTH(forename_3),0))),CHR(92),CHR(92)||CHR(92)),'|',CHR(92)||'|'),CHR(13),CHR(92)||'r'),CHR(10),CHR(92)||'n'),CHR(9),CHR(92)||'t')||CHR(31)||";
				additional_oracle_select_masked="${additional_oracle_select_masked}REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(TO_CLOB(DBMS_RANDOM.STRING('l',NVL(LENGTH(surname),0))),CHR(92),CHR(92)||CHR(92)),'|',CHR(92)||'|'),CHR(13),CHR(92)||'r'),CHR(10),CHR(92)||'n'),CHR(9),CHR(92)||'t')||CHR(31)||'#'";
...
echo "changed_by: ${changed_by}";
							# we will also need to strip any broken bars in
							# the test data
							if [[ $field_name == $changed_by ]] 
							then
echo "field is the changed by";
								# we need to map this to the new
								# modern value
								if [[ $field_nullable == "Y" ]]
								then
...
echo "is notnull"
								...
								if [ $mask_mode == "YES" ] && [ $masked_function != "NONE" ]
								then
									sql_script="${sql_script}REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(TO_CLOB(${masked_function}),CHR(92),CHR(92)||CHR(92)),'|',CHR(92)||'|'),CHR(13),CHR(92)||'r'),CHR(10),CHR(92)||'n'),CHR(9),CHR(92)||'t')"
								else
									if [[ $field_nullable == "Y" ]]
									then
										if [[ $nvl_replacement == "Y" ]]
										then
											sql_script="${sql_script}REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(TO_CLOB(NVL(${field_name},'0')),CHR(92),CHR(92)||CHR(92)),'|',CHR(92)||'|'),CHR(13),CHR(92)||'r'),CHR(10),CHR(92)||'n'),CHR(9),CHR(92)||'t')"
										else
											sql_script="${sql_script}REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(TO_CLOB(NVL(${field_name},'')),CHR(92),CHR(92)||CHR(92)),'|',CHR(92)||'|'),CHR(13),CHR(92)||'r'),CHR(10),CHR(92)||'n'),CHR(9),CHR(92)||'t')"
										fi
									else
										sql_script="${sql_script}REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(TO_CLOB(${field_name}),CHR(92),CHR(92)||CHR(92)),'|',CHR(92)||'|'),CHR(13),CHR(92)||'r'),CHR(10),CHR(92)||'n'),CHR(9),CHR(92)||'t')"
									fi
								fi
							fi
...
echo "checking: ${field_name} ${counter} ${field_count}";
				...
				if [[ ${remapped} == "N" ]]; then
					...
					then
echo "a1";
						sql_script="${sql_script}||CHR(31)||${NEWLINE}";
					else
...
echo "${threads}";
				...
				echo "\"c:\Program Files\PostgreSQL\18\bin\psql.exe\" --set=ON_ERROR_STOP=1 -c \"\copy ${postgres_schema}.${lower_table_name}_temp FROM '${lower_with_schema}_part_${threads}.csv' WITH (FORMAT text, DELIMITER E'\x1f', NULL '')\" \"${postgres_environment}\"">>${postgres_commands_file}_part_${threads}.bat;
...
echo "field_nullable: $field_nullable";
						...
						l_clob_string="REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(TO_CLOB(DBMS_LOB.SUBSTR(b.${field_name},3900,1+(s.piece_no-1)*3900)),CHR(92),CHR(92)||CHR(92)),'|',CHR(92)||'|'),CHR(13),CHR(92)||'r'),CHR(10),CHR(92)||'n'),CHR(9),CHR(92)||'')||CHR(31)||'#'||CHR(31)"
echo "NOTNULL field"
...
echo $field_name
					...
					then
						if [[ $field_type != "CLOB" ]] 
						then
							sql2_script="${sql2_script}||CHR(31)||${NEWLINE}";
						fi
					else
...
echo "ZZ: ${sql_script}";
					...
					sql_script="${sql_script})${NEWLINE}";
					sql_script="${sql_script}SELECT${NEWLINE}";
					sql_script="${sql_script}${sql2_script}||CHR(31)||${NEWLINE}";
...
# Add the script to populate al_ale_sequence_mapping table
echo "INSERT INTO ${postgres_schema}.al_ale_sequence_mapping (">>${postgres_insert_file};
echo "al_id, ale_last_sequence">>${postgres_insert_file};
echo ")">>${postgres_insert_file};
echo "SELECT">>${postgres_insert_file};
echo "al_al_id, MAX(sequence_number)">>${postgres_insert_file};
echo "FROM ${postgres_schema}.application_list_entries">>${postgres_insert_file};
echo "GROUP BY al_al_id;">>${postgres_insert_file};
echo "">>${postgres_insert_file};
...
do
	...
	echo "select 'DROP SEQUENCE IF EXISTS ${postgres_schema}.${sequence_name};' from dual;">>extract_sequences.sql
	echo "select 'CREATE SEQUENCE ${postgres_schema}.${sequence_name} INCREMENT 1 MINVALUE 1 NO MAXVALUE START '||last_number||' CACHE '||cache_size||';' from dba_sequences where sequence_owner = '${sequence_owner}' and sequence_name = '${sequence_name}';">>extract_sequences.sql

done

# ALE_SEQ is a special case, legacy does not use it, modern does, so set it
# to above the maximum current used value
echo "select 'DROP SEQUENCE IF EXISTS ${postgres_schema}.ALE_SEQ;' from dual;">>extract_sequences.sql
echo "select 'CREATE SEQUENCE ${postgres_schema}.ALE_SEQ INCREMENT 1 MINVALUE 1 NO MAXVALUE START '||(max(ale_id)+1)||' CACHE 20;' from appregister.application_list_entries;">>extract_sequences.sql
...
echo "$TABLES_TO_EXTRACT" | tr ',' '\n' | tac | while read -r tables_to_extract; do
	...
	case $tables_to_extract in
		...
		APPREGISTER.APP_LIST_ENTRY_FEE_ID)
			echo "in APP_LIST_ENTRY_FEE_ID"
			primary_key="NVL(TO_CHAR(ALE_ALE_ID),'')||CHR(31)||NVL(TO_CHAR(FEE_FEE_ID),'')||CHR(31)||NVL(TO_CHAR(VERSION),'')||CHR(31)||NVL(TO_CHAR(CHANGED_BY),'')||CHR(31)||NVL(TO_CHAR(CHANGED_DATE,'YYYY-MM-DD HH24:MI:SS'),'')||CHR(31)||REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(NVL(USER_NAME,''),CHR(92),CHR(92)||CHR(92)),'|',CHR(92)||'|'),CHR(13),CHR(92)||'r'),CHR(10),CHR(92)||'n'),CHR(9),CHR(92)||'t') AS row_text";
			delete_allowed="YES";
			...
			delete_statement="WITH parsed AS (
				...
				FROM (SELECT string_to_array(row_text,CHR(31)) AS a
	...
	then
		...
		then
			...
			# Populate the postgres commands file
			echo "\"c:\Program Files\PostgreSQL\18\bin\psql.exe\" --set=ON_ERROR_STOP=1 -c \"\copy ${postgres_schema}.${lower_table_name}_delete_temp FROM '${lower_with_schema}.deletes.csv' WITH (FORMAT text, DELIMITER E'\x1f', NULL '')\" \"${postgres_environment}\"">>$postgres_delete_commands_file;
