-- V57__arcpoc-1435-correct_logging_of_job_in_data_audit.sql

-- Version Control
-- V1.0  	Matthew Harman  09/06/2026	Initial Version
--

-- Drop the existing write_data_audit_table procedure if it exists
DROP PROCEDURE IF EXISTS write_data_audit_table(text, text, text, text, text, text, text);

-- Create the new write_data_audit_table procedure with the correct logging of job information
...
BEGIN
	IF p_clob = 'N' THEN
		INSERT INTO ${flyway:defaultSchema}.data_audit(data_id,
									  schema_name,
									  table_name,
									  column_name,
									  old_value,
...
-- Create the new version of the function delete_expired_application_lists_core
					...
					IF NOT p_return_count THEN
						IF l_data_audit THEN
							...
							CALL ${flyway:defaultSchema}.write_data_audit_table('${flyway:defaultSchema}'::text,
														...
														l_database_job::text,
														'Scheduled Job'::text,
														r_application_list.al_id::integer,
    													'N'::text);
